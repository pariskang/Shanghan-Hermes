...
**接入智能體框架**（22 個只讀回源工具 + 1 個智能體工具，三種 harness 共用同一能力面；
...
民國，歷時約 1800 年）對宋本條文的引用逐字檢測為 **40,660 條引文邊**
（聚合為 13,405 個著作×條文對），按「引述標記 × 覆蓋率」判定七種引用模式
...
- **引文識別自檢基準**：合成明引/節引/暗引 檢出率 1.00、改寫 1.00（≤45 字
  條文）、負例誤報 0.00（確定性下限標尺）。詳見 [`docs/TRACE.md`](docs/TRACE.md)。
...
├─ agent/       tools(22 工具+ScopedRegistry+患者白名單+結果緩存) · citation_guard
