...
另有 **🌸 粉晶 Gradio 界面**（醫哲未來人工智能研究院 · Rose-Quartz Studio）：
對話研習（單智能體/任務圖編排/多智能體合議 × 四角色，右側檢索原文/多假設/
合議/核驗四聯面板）· 深度研究檔案 · 方證工具台 · 評測基準，支持結果導出與
ngrok 公網映射：

```bash
pip install "hermes-shanghan[webui]"     # gradio + pyngrok
python3 -m hermes_shanghan webui         # http://127.0.0.1:7860/
python3 -m hermes_shanghan webui --ngrok-token <token>   # 公網鏈接
```
...
# 深度溯源·知識生命史（源頭→異文→注家→後世→現代映射；六種引用模式；
# 古今影響力指標；現代文獻計量離線誠實 deferred，不虛構論文）
python3 -m hermes_shanghan tool-call shanghan_provenance --args '{"ref":"12"}'
python3 -m hermes_shanghan tool-call shanghan_provenance --args '{"concept":"腎主骨"}'
...
# 🌸 粉晶 Gradio 界面（醫哲未來人工智能研究院；需 pip install "hermes-shanghan[webui]"）
python3 -m hermes_shanghan webui                 # http://127.0.0.1:7860/
...
# 測試（302 項：對抗性審核 + 智能體架構 + 27 工具 + 評測 + 六維研究循環 + 全庫接入
#       + 可復現性/證據鏈硬化 + Colab守衛）
...
**接入智能體框架**（27 個只讀回源工具 + 1 個智能體工具，三種 harness 共用同一能力面；
...
取證並產出引用核驗的研究發現 → **批評家**檢查五大溯源維度（原文源流/異文
注家/方證計量/劑量計量/客觀評測）缺口，未覆蓋維度進入下一輪 → 收斂或達
最大輪數。溯源檔案（dossier）驅動第 8 種論文類型 `provenance`：循環軌跡表
+ 逐條核驗的溯源發現表 + 增益層綜合。
...
│               · decoction(煎服法方後原文解析) · correspondence(方證對應引擎)
│               · provenance(深度溯源：知識生命史+六引用模式+影響力指標+學術連接器)
├─ rag/         bm25 · clause_rag（原文RAG）· omni_search（全景多路檢索）
│               · vector_channel（語義向量：嵌入增益/TF-IDF兜底）
│               · skill_rag（技能路由）
...
│               · commentary_atlas（注家分歧圖譜）· webui（粉晶 Gradio 界面）
...
├─ agent/       tools(27 工具+ScopedRegistry+患者白名單+結果緩存) · citation_guard
...
tests/          302 項測試 ｜ notebooks/ Colab 全功能演示（守衛測試保證與代碼同步）
