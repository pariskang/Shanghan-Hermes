...
## 深度溯源引擎（provenance）——從「查出處」到「追蹤知識生命史」

工具 `shanghan_provenance`（入口：條文號 / 方名 / 概念 / 待溯源文本）把一個
知識單元的完整演化鏈串成一次可解釋輸出：

```
原始條文(A) → 版本異文(B) → 注家解釋(C) → 後世方論醫案(旁證)
           → 現代疾病映射(D,候選) → 現代機制橋接 → 科研熱點
```

- **不建 9 個新智能體**：溯源是把既有工具（get_clause/variants/
  divergence_atlas/relations/correspondence/library）編排為一條知識鏈——
  一個 `ProvenanceTracer` 復用它們，能力相同而無編排臃腫；
- **六種引用模式識別**（`classify_citation`）：明引（《書名》曰+逐字）/
  暗引（逐字無標記）/節引（連續片段，最長連續run≥5）/改寫（語義近逐字疏）/
  轉引（經後世注本醫案中轉）/誤引（標稱條號與最佳匹配不符）——標點/字形
  變體魯棒；
- **古今知識影響力指標**（§確定性，逐項附依據）：源頭度、傳承度（注家承襲
  數 + 全庫反響）、異文穩定度（版本相似度）、注家爭議度（**直取九注本
  對齊層真實計量**，第12條 0.9167）、方證擴展度（關係圖譜/加減方數）、
  現代轉化度（表型映射等級）、證據可靠度；
- **現代文獻計量走可插拔連接器，離線誠實降級**：`ScholarConnector` 協議 +
  `NullScholarConnector`（離線默認）。無外部引文源時 `core_papers/authors`
  一律為空並標 `deferred：需接入 Crossref/OpenAlex/Semantic Scholar/
  OpenCitations`——**絕不虛構論文/DOI**（原方案自己把幻覺率列為失敗指標）；
  接入真實連接器後由外部數據填充。與既有 litellm/向量增益層同構；
- **誠實邊界**：傷寒論未直述的更早命題（如「腎主骨」源出《內經》）標
  `deferred_earlier_classic`、confidence 0，不硬湊出處，仍給全庫後世反響與
  現代映射；輸出全程區分原文（A）/注家（C）/後世發揮（D）/現代映射（候選），
  古籍概念與現代疾病、分子機制**不機械等同**（腎虛≠Wnt、血瘀≠血栓）。

路由：「第12條的知識生命史/傳播路徑？」「桂枝湯的源流演化？」→ provenance；
與 get_clause（問文本）、perspectives（問詮釋）、divergence_atlas（問計量）
互不侵佔。研究/患者角色隔離照舊（provenance 不在 PATIENT_SAFE_TOOLS）。
...
`shanghan_provenance`（深度溯源：知識生命史+六種引用模式+古今影響力指標+
學術連接器誠實降級）、
...
```
暴露上述 27 個工具 + `shanghan_ask`（完整智能體）。MCP 服務器為純標準庫實現的
