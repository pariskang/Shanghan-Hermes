...
`shanghan_mistreatment`、`shanghan_list_formulas`，以及十個**研究/推理模塊**：
...
```
暴露上述 18 個工具 + `shanghan_ask`（完整智能體）。MCP 服務器為純標準庫實現的
...
hermes_shanghan/agent/       tools(18+ScopedRegistry) · citation_guard · agent(ReAct+反思)
